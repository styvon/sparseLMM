...
glmmkin.ai_PCG_Rcpp_Binary = function(genofile, fit0, tau=c(0,0), fixtau = c(0,0), maxiter =20, tol = 0.02, verbose = TRUE, nrun=30, tolPCG = 1e-5, maxiterPCG = 500, subPheno, obj.noK, out.transform, tauInit, memoryChunk, LOCO, chromosomeStartIndexVec, chromosomeEndIndexVec, traceCVcutoff) {
  ...
  mu.eta = family$mu.eta(eta) # derivative: dmu/deta
  Y = eta - offset + (y - mu)/mu.eta # working vector
  alpha0 = fit0$coef
  eta0 = eta
...
#' @param kinFile (new)character. Path to the kinship matrix file
#' @param sparse_kin (new)Logical, whether kinship matrix is stored as sparse form (MatrixMarket format), default is FALSE
#' @param nThreadsOMP (new)integer, number of threads for OpenMP with external sparse GRM, default is 1
...
fitNULLGLMM = function(plinkFile = "", 
  ...
  cat(nThreadsOMP, " threads are set for OpenMP with external sparse GRM\n")
  
  #check and read files
  #output file
  modelOut=paste0(outputPrefix, ".rda")
  if(sparse_kin){
    SPAGMMATOut=paste0(outputPrefix,"_", numMarkers,"markers.sparseLMM.results.txt")
  }else{
  ...
  } # end of if(!file.exists(phenoFile))
...
# YZ: add sparse kinship params 2019/03/15
scoreTest_SPAGMMAT_forVarianceRatio_binaryTrait <-  function(obj.glmm.null,
  ...
  if(sparse_kin){
    cat("Using stored sparse kinship in computation\n")  
    Phi = readMM(kinFile)
  }
  ...
  mu.eta = family$mu.eta(eta) # derivative d mu / d eta
  ...
  if(sparse_kin){
    Sigma_iX_noLOCO = getSigma_X_sparse(W, tauVecNew, X1, maxiterPCG, tolPCG, Phi, mMarkers, nThreadsOMP)
  }else{
    Sigma_iX_noLOCO = getSigma_X(W, tauVecNew, X1, maxiterPCG, tolPCG) 
  }
    
    
  y = obj.glm.null$y # y is output, for progressive: phenotype
    ...
    while(numTestedMarker < numMarkers){
      ...
      if (AC <= 20 | AC >= (2*Nnomissing - 20) | CHR < 1 | CHR > 22){
        ...
        if(!obj.glmm.null$LOCO){
          if(sparse_kin){
            Sigma_iG = getSigma_G_sparse(W, tauVecNew, G, maxiterPCG, tolPCG, Phi, mMarkers, nThreadsOMP) 
