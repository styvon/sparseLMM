/* This code is modified based on SAIGE source code from Wei Zhou*/
...
//YZ added for computation using stored sparse kinship mat
// [[Rcpp::export]]
arma::fvec getDiagOfSigma_sparse(arma::fvec& wVec, arma::fvec& tauVec, const arma::sp_mat& phi){
	...
	// float floatBuffer;
   	...
   	arma::vec diagPhi(phi.diag());
   	arma::fvec fdiagPhi = arma::conv_to< arma::fvec >::from( diagPhi );

   	
	diagVec = tauVec(1)* fdiagPhi  + tauVec(0)/wVec;
	for(unsigned int i=0; i< Nnomissing; i++){
  		if(diagVec(i) < 1e-4){
  			diagVec(i) = 1e-4 ;
  		}
...
arma::fcolvec getCrossprod_sparse(arma::fcolvec& bVec, arma::fvec& wVec, arma::fvec& tauVec, const arma::sp_mat& phi){
        ...
        arma::vec crossProd1(phi * bVec);
        arma::fvec fcrossProd1 = arma::conv_to< arma::fvec >::from( crossProd1 );

        crossProdVec = tauVec(0)*(bVec % (1/wVec)) + tauVec(1)*fcrossProd1;
...
// YZ: add for computation using stored sparse kinship matrix with OMP
// Function to get outvec = sigma %*% x
// sigma = tau[0] * diag(w) + tau[1] * phi
void SigmaVecmult_sparse(const fvec& wVec, const fvec& tauVec, const sp_mat& phi, const fvec *x, fvec *outvec, int nrows, int nThreads){
  ...
  #pragma omp parallel 
  {
    ...
    for(i=0; i<nrows; i++){
      rowsum = 0.0;
      sp_mat::const_row_iterator it = phi.begin_row(i);
      sp_mat::const_row_iterator it_end = phi.end_row(i);
      while(it != it_end){
        rowsum += (*it) * (*x)( it.col() ) * tauVec(1)  ;
        // cout << (*it) << endl;
        ++it;
      }
      (*outvec)(i) = rowsum + wVec(i) * tauVec(0) * (*x)(i);
...
// YZ: add for computation using stored sparse kinship matrix with OMP
arma::fvec getPCG1ofSigmaAndVector_sparse(arma::fvec& wVec,  arma::fvec& tauVec, arma::fvec& bVec, int maxiterPCG, float tolPCG, const arma::sp_mat& phi, int nThreads=1 ){ //!under construction
  	...
  	arma::fvec MinvVec = 1/getDiagOfSigma_sparse(wVec, tauVec, phi);


  	arma::fvec z = MinvVec % r;
  	arma::fvec z1Vec;
 	arma::fvec p = z;
  	...
  	arma::fvec Ap(Nnomissing);
  	float alpha, beta;

  	//pointers
  	arma::fvec* p_Ap = &Ap;   //pointer to Ap
  	const arma::fvec* p_p = &p; //pointer to p
  
  	int iter = 0;
  	// while (sumr2 > tolPCG && iter < maxiterPCG) {
  	while (rz_new > tolPCG && iter < maxiterPCG) {
    		iter = iter + 1;
    		// arma::fcolvec ApVec = getCrossprod_sparse(pVec, wVec, tauVec, phi);
    		SigmaVecmult_sparse(wVec, tauVec, phi, p_p , p_Ap, Nnomissing, nThreads); // get Ap!!, new param: wVec, tauVec, phi
...
/* YZ: for stored sparse grm*/
// [[Rcpp::export]]
arma::fmat getSigma_X_sparse(arma::fvec& wVec, arma::fvec& tauVec,arma::fmat& Xmat, int maxiterPCG, float tolPCG, const arma::sp_mat& phi, int M, int nThreads=1){
...
/* YZ for computation using stored sparse kinship mat*/
// [[Rcpp::export]]
arma::fvec  getSigma_G_sparse(arma::fvec& wVec, arma::fvec& tauVec,arma::fvec& Gvec, int maxiterPCG, float tolPCG, const arma::sp_mat& phi, int M,int nThreads=1){
  	arma::fvec Sigma_iG;
  	Sigma_iG = getPCG1ofSigmaAndVector_sparse(wVec, tauVec, Gvec, maxiterPCG, tolPCG, phi, nThreads)/M;
  	return(Sigma_iG);
}
